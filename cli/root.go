// Package cli builds the lobsters command tree on top of the lobsters library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: lobsters.DefaultConfig()}

	root := &cobra.Command{
		Use:   "lobsters",
		Short: "A command line for Lobste.rs.",
		Long: `lobsters reads public Lobste.rs data: story listings, single stories with
their comment trees, tag feeds, and user profiles. No account or API key needed.
Records come back as table, JSON, JSONL, CSV, TSV, or URLs.

lobsters is an independent tool and is not affiliated with Lobste.rs or its operators.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.IntVarP(&app.limit, "limit", "n", 0, "limit number of records (0 = feed default)")
...
func (a *App) setup() error {
	...
	a.client = lobsters.NewClient(a.cfg)
	return nil
}
...
func mapFetchErr(err error) error {
	...
	if isRateLimited(err) {
		return codeError(exitRateLimited, fmt.Errorf("%w (try --delay or --retries)", err))
	}
